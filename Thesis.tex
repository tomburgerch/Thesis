...
\textbf{Evaluation: The Forward-Backward algorithm} is a recursive solution. A first approach to evaluate, which model has produced a certain set of observations might be the following:
\begin{enumerate}
...
$$a_i(t) = \begin{cases} \pi_i b_i(v(0)), & \mbox{t = 0 } \\ \sum_{j=1}^N a_j(t-1)a_{ij}b_j (v(t))& \mbox{t \ne } \end{cases}$$

\item So for $t=0$ we initialize $a$
$$a_i(0) = \pi_i b_i (v(0))$$

\item We compute $a$ for all states $i$ at all times $t$
\item The total probability that a model $M$ generated the sequence $V^T$ results from the sum of all $a_i(T)$

$$P(V^T|M) = \sum\limits_{i=1}^N a_i(T)$$
\end{enumerate}



\textbf{Training: The Baum-Welch-Algorithm} is used to train signature model. A detailed description is given by Rabiner et al. in \ref{rabiner1989tutorial}. The algorithm's output is the transition matrix $A$ that is most likely to generate the given sequence of observations. The following needs to be known to start the algorithm:
\begin{itemize}

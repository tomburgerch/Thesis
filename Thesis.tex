...
We defined the start vecor $W = [0,0,0,0,0,0,0]$ and let the fitness function permutate the vector until a local minimum was found for all $W$ consisting of $\{0,1,2\}$. The local minima was reached for $$W = [w_l, w_t, w_{v_\text{max}}, w_{\bar{v}}, w_{a_\text{max}}, w_n, w_r] = [1, 1, 1, 1, 0, 0, 0]$$
At the local minimum, there are 1007 signatures with the highest score which is 696 more than in the ground truth set that we used for this technique.

If we extend our search and include the two groups with the highes occurrencies, we get


\textbf{Detection of forgeries}\\
We used the previously found vector to test the robustness of this approach against forgeries. We set $$W = [1, 1, 1, 1, 0, 0, 0]$$
and ran all the forgeries against the previously trained signature models given by the minimum and maximum values.


SLIDING WINDOW ERWAEHNEN!

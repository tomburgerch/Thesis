...
The first line corresponds to the case of insertion, the second line to a match and the third line to an insertion. If in each step, there is a match, the path will go along the matrix' diagonal. Generally, the if the path is close to the diagonal, the signature's are similar and the score thus small.
...
\begin{itemize}
\item Given the model parameters and observed data, estimate the optimal sequence of hidden states.
\item Given the model parameters and observed data, calculate the likelihood of the data.
\item Given just the observed data, estimate the model parameters.
\end{itemize}
